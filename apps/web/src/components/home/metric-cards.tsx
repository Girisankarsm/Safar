...
export function MetricCards() {
  ...
  useEffect(() => {
    ...
    Promise.all([api.cctv(city, lat, lng), api.reports(city), api.wallet()])
      .then(([c, r, w]) => {
        const cctvCount = c.count || c.nodes?.length || 0;
        setMetrics({
          safetyIndex: Math.min(95, 72 + Math.min(20, Math.floor(cctvCount / 15))),
          cctv: cctvCount,
