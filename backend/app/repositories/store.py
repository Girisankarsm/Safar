...
_mem: dict[str, Any] = {
    ...
    "reports": [],
    "sos": [],
    "wallet": {"balance": 340, "lifetime_tokens": 520, "lifetime_co2_kg": 12.4, "green_trips": 28},
    "transactions": [],
}
...
def is_db_active() -> bool:
    return _db_ready and SessionLocal is not None
...
def get_current_user() -> dict:
    if is_db_active():
        with SessionLocal() as db:
            user = db.get(User, _uid(settings.demo_user_id))
            if user:
                return {
                    "id": str(user.id),
                    "name": user.name,
...
def update_user_prefs(prefs: dict) -> dict:
    ...
    if is_db_active():
        with SessionLocal() as db:
            row = db.get(User, _uid(settings.demo_user_id))
            if row:
...
def get_route(route_id: str) -> dict | None:
    if is_db_active():
        with SessionLocal() as db:
            row = db.get(Route, _uid(route_id))
            if not row:
                return None
...
def start_trip(route_id: str) -> dict | None:
    ...
    if is_db_active():
        with SessionLocal() as db:
            db.add(Trip(
                id=_uid(tid),
                user_id=_uid(settings.demo_user_id),
...
def get_trip_by_share(share_token: str) -> dict | None:
    if is_db_active():
        with SessionLocal() as db:
            row = db.scalar(select(Trip).where(Trip.share_token == share_token))
            if row:
    ...
    if is_db_active():
        with SessionLocal() as db:
            ...
            if row:
                ...
                db.commit()
            wallet = db.scalar(select(CarbonWallet).where(CarbonWallet.user_id == _uid(settings.demo_user_id)))
            if wallet:
...
def reports_near(lat: float, lng: float, city: str, radius_km: float = 1.0) -> list[dict]:
    ...
    if is_db_active():
        with SessionLocal() as db:
            rows = db.scalars(select(SafetyReport).where(SafetyReport.city == city).limit(200)).all()
            for r in rows:
...
def get_wallet() -> dict:
    if is_db_active():
        with SessionLocal() as db:
            w = db.scalar(select(CarbonWallet).where(CarbonWallet.user_id == _uid(settings.demo_user_id)))
            if w:
                txs = db.scalars(
...
def get_contacts() -> list[dict]:
    if is_db_active():
        with SessionLocal() as db:
            rows = db.scalars(
                select(EmergencyContact).where(EmergencyContact.user_id == _uid(settings.demo_user_id))
            ).all()
